...
def init_db():
    ...
    default_items = [
        ("In welcher Form sind die Angebote/Teilnahmeanträge einzureichen?", "question"),
    ...
    for text, item_type in default_items:
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute('INSERT INTO questions (text, type) VALUES (?, ?)', (text, item_type))
...
async def debug_cors():
    ...
    return {
        ...
        "allowed_origins": allowed_origins if not IS_PRODUCTION else ["hidden for security"],
        "cors_enabled": True
    }
...
async def debug_info():
    return {
        ...
        "environment": ENVIRONMENT,
        "is_production": IS_PRODUCTION,
        "allowed_cors_origins": allowed_origins if not IS_PRODUCTION else ["hidden in production"],
        "api_version": "1.0.0"
    }
...
async def chat_with_claude(request: ChatRequest):
    ...
    try:
        ...
        if request.file_ids:
            ...
            cursor.execute(f'SELECT filename, anthropic_file_id FROM files WHERE id IN ({placeholders})', request.file_ids)
...
Respond in JSON format as specified in the system prompt."""
        ...
        raise HTTPException(status_code=500, detail=f"Checklist processing failed: {str(e)}")
